...
/**
 * BLE transport for MeshcoreCompanion using the Nordic UART Service (NUS).
 * Uses SimpleBLE (simplejavable) for cross-platform BLE support.
 *
 * Download simplejavable.jar from https://github.com/simpleble/simpleble/releases
 * and place at lib/simplejavable.jar. Also add the platform native lib
 * (libsimplejavable.so / libsimplejavable.dll) to java.library.path.
 *
...
public class BleMeshcoreCompanion extends MeshcoreCompanion {
    ...
    private void connectBle() throws IOException {
        ...
        try {
            ...
            try { found.disconnect(); } catch (Exception ignore) {}
    ...
    private void disconnectBle() {
        ...
        peripheral = null;
        if (p != null) {
            try { p.disconnect(); } catch (Exception ignore) {}
        }
    }
    ...
    /**
     * Parse one complete frame payload from rxBuffer.
     * Resyncs on the '>' start byte, returns payload (without header) on success,
     ...
     * visible peripherals. Use this to discover the address to pass to the constructor.
     *
